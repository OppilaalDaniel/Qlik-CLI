...
if (Test-Path -Path "C:\Users\oppilaale\Documents\BackUps\$($backupDatefolder)")    #Change folder path
{
Get-ChildItem "C:\Users\oppilaale\Documents\BackUps\$($backupDatefolder)" -Include *.* -Recurse | ForEach  { $_.Delete()}    #Change folder path
Remove-Item "C:\Users\oppilaale\Documents\BackUps\$($backupDatefolder)" -Recurse -Force    #Change folder path
} 
New-Item -Path "C:\Users\oppilaale\Documents\BackUps\$($backupDatefolder)" -ItemType Directory    #Change folder path

#Get ManagedSspace Names and Ids
$Sharedspace= qlik space ls --type shared --limit 100 | ConvertFrom-Json
$SharedspaceId=$Sharedspace.id
$SharedspaceName=$Sharedspace.name
$spacecount=$SharedspaceId.Count
for($i=0 ; $i -lt $spacecount ; $i++)
{

New-Item -Path "C:\Users\oppilaale\Documents\BackUps\$($backupDatefolder)\$($SharedspaceName[$i])" -ItemType Directory    #Change folder path

$apps=qlik app ls --limit 100 --spaceId $SharedspaceId[$i] --quiet
echo $apps
echo $SharedspaceName[$i]
...
$backupLocation = "C:\Users\oppilaale\Documents\BackUps\$($backupDatefolder)\$($SharedspaceName[$i])"    #Change folder path
